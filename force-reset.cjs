...
async function run() {
  try {
    ...
    console.error("Erro no BD:", e.message);
  }
}
run();
`;
...
conn.on('ready', () => {
  console.log('Conectado à VPS. Arrumando banco de dados e reiniciando...');
  conn.exec(`cat << 'EOF' > /root/fybot/fix-pwd.js\n${remoteScript}\nEOF\ncd /root/fybot && node fix-pwd.js && pm2 restart fybot`, (err, stream) => {
    if (err) throw err;
