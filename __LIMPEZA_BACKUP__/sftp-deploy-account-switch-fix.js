...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    ...
    const blockToReplace = `    if (balance && state.balance !== balance) {
      state.balance = balance;
    }`;

    const newBlock = `    if (balance && state.balance !== balance) {
      // If the balance changes drastically, it means the user switched accounts
      if (Math.abs((state.balance || 0) - balance) > 50) {
          state.customStartingBalance = balance;
      }
      state.balance = balance;
    }`;
