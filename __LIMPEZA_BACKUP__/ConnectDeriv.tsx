  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (savedUserStr) {
        ...
        if (!response.ok) {
          throw new Error(`Falha ao salvar token (status ${response.status})`);
        }

        // Recarrega com o token na URL para o App.tsx processar.
        // Valores codificados para não quebrar a query string caso o token
        // contenha caracteres especiais (&, =, #, %, etc).
